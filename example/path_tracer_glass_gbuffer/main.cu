...
CUDA_INLINE CUDA_HOSTDEVICE float3 m_reflect(float3 v, float3 N) {
    float ndotv = dot(v, N);
    return -v + 2 * ndotv * N;
}

CUDA_INLINE CUDA_HOSTDEVICE float3 m_refract(float3 v, float3 N, float ior) {
    float cosi = dot(v, N);
    
    float eta = (cosi > 0.0f) ? (1.0f / ior) : ior;
    
    float k = 1.0f - eta * eta * (1.0f - cosi * cosi);
    
    if (k < 0.0f) {
        return float3(0.0f, 0.0f, 0.0f);  // 全反射，返回零向量表示
    }
    
    float3 refracted = eta * v - (eta * cosi + sqrtf(k)) * N;
    return refracted;
}

CUDA_INLINE CUDA_HOSTDEVICE float m_fresnel(float cosTheta, float ior) {
    float eta = ior;

    float sin2theta_t = eta * eta * (1.0f - cosTheta * cosTheta);
    if (sin2theta_t > 1.0f) {
        // 全反射情况
        return 1.0f;
    }
    ...
    float Rs = (cosTheta - eta * cosTheta_t) / (cosTheta + eta * cosTheta_t);
    Rs = Rs * Rs;

    float Rp = (eta * cosTheta - cosTheta_t) / (eta * cosTheta + cosTheta_t);
    Rp = Rp * Rp;
...
struct PathPayloadRecord {
    ...
    float3 reflect_position;
    float3 refract_position;
    float fresnel;
    float miss;
...
extern "C" __global__ void __raygen__main() {
    ...
    if (!record.done) {
        ...
        optix_launch_params.normal_buffer[pixel_index] = make_float3(0.f, 0.f, 0.f);
    }

    optix_launch_params.test[pixel_index] = record.random.Next();

    float distance = 10000.f;
    float3 ray_origin_origin = ray_origin;

    while (!record.done) {
        record.radiance = make_float3(1.f, 0.f, 0.f);
        ++depth;
...
////
        ...
        // bsdf sampling
        {
            ...
            if(depth == 1){
                record.reflect_direction = m_reflect(-ray_direction, local_hit.geo.normal);
                record.fresnel = bsdf_sample_record.fresnel;
                record.reflect_position = ray_origin;
                
                float3 offset = record.hit.geo.position - ray_origin_origin;
                distance = 
                    ray_direction.x > 1e-6? offset.x / ray_direction.x : 
                    ray_direction.y > 1e-6? offset.y / ray_direction.y : offset.z / ray_direction.z;
                //record.first_distance = record.distance;
            }
            else if(depth == 2){
                record.refract_direction = normalize(ray_direction);
                record.refract_position = ray_origin;
                break;
            }
...
//
    ...
    optix_launch_params.reflect_buffer[pixel_index] = make_float4(record.reflect_direction, record.fresnel);
    optix_launch_params.refract_buffer[pixel_index] = make_float4(record.refract_direction, 1.f - record.miss);
    optix_launch_params.reflect_position_buffer[pixel_index] = make_float4(record.reflect_position, distance);
    optix_launch_params.refract_position_buffer[pixel_index] = make_float4(record.refract_position, 1.f);
    //optix_launch_params.reflect_buffer[pixel_index] = make_float4(optix_launch_params.normal_buffer[pixel_index], 1.f);
    //optix_launch_params.refract_buffer[pixel_index] = make_float4(optix_launch_params.normal_buffer[pixel_index], 1.f);
}
