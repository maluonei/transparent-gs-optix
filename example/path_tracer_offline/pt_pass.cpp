...
int saveToEXR(cuda::RWArrayView<float4> view, int width, int height, const std::string &filename) {
    ...
    // 将 CUDA 内存中的数据复制到 rgbaData
    float4 *cudaDeviceData = view.GetDataPtr();// 假设 view.data() 返回设备内存指针
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            // 将 float4 转换为 TinyEXR 格式的 RGBA 浮点数组
...
int saveToEXR(cuda::RWArrayView<float3> view, int width, int height, const std::string &filename) {
    ...
    // 将 CUDA 内存中的数据复制到 rgbaData
    float3* cudaDeviceData = view.GetDataPtr();// 假设 view.data() 返回设备内存指针
...
void PTPass::SaveFrame(const std::filesystem::path& path) noexcept {
    try {
        // 检查文件夹是否存在
        if (!std::filesystem::exists(path)) {
            // 如果文件夹不存在，创建它
            if (std::filesystem::create_directory(path)) {
                std::cout << "文件夹已创建: " << path << std::endl;
            } else {
    ...
    std::filesystem::path normal_path = path / "normal.exr";
    //std::filesystem::path refract_path = path / "refract.exr";
